...
const CalendarManager = {
    ...
    render(type = 'general') {
        ...
        for (let d = 1; d <= totalDiasMes; d++) {
            ...
            if (stateObj.diaSeleccionado === d) {
                ...
                divDia.style.backgroundColor = 'var(--cream, #fff9e6)';
                divDia.style.fontWeight = '700';
            }
        ...
        // 3. Sincronizar input de fecha
        const inputId = type === 'general' ? 'inputFechaGeneral' : ('inputFecha' + suf);
        const elInput = document.getElementById(inputId);
        if (elInput) {
            if (stateObj.diaSeleccionado) {
                const mesPad = String(mes + 1).padStart(2, '0');
                const diaPad = String(stateObj.diaSeleccionado).padStart(2, '0');
                elInput.value = `${año}-${mesPad}-${diaPad}`;
            }
            this.configurarInputFecha(type);
        }
    ...
    // Configurar listener para el selector de fecha
    configurarInputFecha(type) {
        const suf = this.getSuffix(type);
        const inputId = type === 'general' ? 'inputFechaGeneral' : ('inputFecha' + suf);
        const elInput = document.getElementById(inputId);
        if (elInput && !elInput.dataset.calendarListener) {
            elInput.dataset.calendarListener = 'true';
            elInput.addEventListener('change', (e) => {
                this.irAFecha(type, e.target.value);
            });
        }
    },

    // Ir directamente a una fecha seleccionada (YYYY-MM-DD)
    irAFecha(type, fechaStr) {
        if (!fechaStr) return;
        ...
        if (partes.length === 3) {
            ...
            if (!isNaN(año) && !isNaN(mes) && !isNaN(dia)) {
                const stateObj = this.state[type] || this.state.general;
                stateObj.fecha = new Date(año, mes, 1);
...
function irAFecha(fechaStr) { CalendarManager.irAFecha('general', fechaStr); }
...
function irAFechaClub(club, fechaStr) { CalendarManager.irAFecha(club, fechaStr); }
...
window.addEventListener('supabase_synced_eventosIglesia', () => {
    CalendarManager.render('general');
});
window.addEventListener('supabase_synced_eventos_iglesia', () => {
    CalendarManager.render('general');
});
window.addEventListener('supabase_connection_status', (e) => {
    if (e.detail && e.detail.ok) {
        CalendarManager.sincronizarConSupabase();
    }
});
